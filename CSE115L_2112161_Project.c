...
void add(struct good arr[], int number)
{
    ...
    printf("You should save this file afterwards\n");
    printf("Name: ");
    fflush(stdin);
...
void check_pass()
{
    char username[20] = "rahat";
	char password[20] = "2112161642";

	char input_un[20], input_pass[20];
	...
	while( strcmp(username, input_un) != 0 || strcmp(password, input_pass) != 0 )
    {
...
void save(struct good arr[], int number)
{
    FILE *fp;
    ...
    for(i = 0 ; i < number ; i++)
    {
        fprintf(fp, "%s\n%s\n%f\n%d\n", arr[i].ID, arr[i].name, arr[i].rate, arr[i].quantity);
    }
